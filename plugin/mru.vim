...
" Check file readable when load list
if !exists('MRU_Check_File')
    let MRU_Check_File = 0
endif
...
function! s:MRU_AddFile(acmd_bufnr)
    ...
    let idx = index(s:MRU_files, fname, 1)
    if idx == -1
...
function! s:MRU_Open_Window(...)
	...
	" changed by StarWing
        silent! 0put =filter(copy(s:MRU_files), 'filereadable(v:val)')
    else
        " Display only the entries matching the specified pattern
        let pat = '\V'.substitute(a:1, '\\', '\\\\', 'g')
        silent! 0put =filter(copy(s:MRU_files), 'v:val =~? pat && filereadable(v:val)')
    endif
...
function! s:MRU_Refresh_Menu()
    ...
    " Add By StarWing
    if g:MRU_Check_File
	let MRU_files = copy(s:MRU_files)
