...
"
  ...
  if has("vms")
    ...
    " exe "%!" . g:xxdprogram . " -r"
    " 07Aug03 Fix
    exe '%!' . shellescape(g:xxdprogram) . ' -r'
  endif
  set ft=
...
"
  if !exists("g:xxdprogram")
    ...
    if (has("win32") || has("dos32")) && !executable("xxd")
      let g:xxdprogram = $VIMRUNTIME . '\xxd.exe'
    else
...
function s:HEX_Status()
  ...
  if g:hex_showstatus == 1
    ...
    " switch on 
    :highlight AsciiPos guifg=Black guibg=Yellow cterm=reverse term=reverse
    let s:sff = s:HEX_ShowOffsets()
    let s:fff = s:sff + 1

    " 29Mrz06 FR vim 7.0c don't accept /\%<colpos>v
    :au! Cursorhold * let s:curoffset = s:HEX_ShowOffsets()
                \| exe 'match AsciiPos /\%<' . (s:curoffset + 1) . 'v.\%>' . s:curoffset . 'v/'
    :set ut=100 
